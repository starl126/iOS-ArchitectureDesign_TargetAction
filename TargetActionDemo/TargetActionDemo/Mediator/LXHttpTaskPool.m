...
/// 任务池操作回调
@property (nonatomic, copy) void (^taskPoolActionCallback)(NSString* msg, LXTaskCRUDResultType code);

/// 任务请求完成回调
@property (nonatomic, copy) void (^taskCompletedCallback)(LXHttpTaskModel* _Nonnull task,LXHttpResData* resData);
/// 任务上传实时回调
@property (nonatomic, copy) void (^taskUploadProgressCallback)(LXHttpTaskModel* _Nonnull task, NSProgress* _Nonnull progress);
/// 任务下载实时回调
@property (nonatomic, copy) void (^taskDownloadProgressCallback)(LXHttpTaskModel* _Nonnull task, NSProgress* _Nonnull progress);
...
- (void)p_actionForAddTask:(LXHttpTaskModel*)task {
    ...
    if (exist == 1) {//进行中任务池中存在
        ...
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"该任务已经存在于进行中任务池", LXTaskCRUDResultTypeExist);
        }
        return;
    }
    ...
    if (exist == 2) {
        ...
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"该任务已经存在于暂停中任务池", LXTaskCRUDResultTypeExist);
        }
        return;
    }
    ...
    if (exist == 3) {
        ...
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"该任务已经存在于等待中任务池", LXTaskCRUDResultTypeExist);
        }
        return;
    }
    ...
    if (exist == 0) {
        if (self.runningTaskPool.count >= self.maxRunningTaskCount && self.waitingTaskPool.count >= self.maxWaitingTaskCount) {
            ...
            if (self.taskPoolActionCallback) {
                self.taskPoolActionCallback(@"所有任务池爆满,请检查会话所有请求状态", LXTaskCRUDResultTypeAddFail);
            }
            return;
        }
        if (self.runningTaskPool.count >= self.maxRunningTaskCount && self.waitingTaskPool.count < self.maxWaitingTaskCount) {
            ...
            if (self.taskPoolActionCallback) {
                self.taskPoolActionCallback(@"进行中任务池爆满,任务存放于等待中任务池", LXTaskCRUDResultTypeAddSuccess);
            }
        ...
        if (self.runningTaskPool.count < self.maxRunningTaskCount) {
            ...
            if (self.taskPoolActionCallback) {
                self.taskPoolActionCallback(@"任务存放于进行中任务池", LXTaskCRUDResultTypeAddSuccess);
            }
...
- (void)p_actionForDeleteTaskIdentifier:(NSString*)identifier {
    ...
    if (success) {
        ...
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"进行中任务删除成功", LXTaskCRUDResultTypeDeleteSuccess);
        }
        return;
    }
    ...
    if (success) {
        ...
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"等待中任务删除成功", LXTaskCRUDResultTypeDeleteSuccess);
        }
        return;
    }
    ...
    if (success) {
        ...
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"暂停中任务删除成功", LXTaskCRUDResultTypeDeleteSuccess);
        }
    }else {
        LXLog(@"警告⚠️:任务池中不存在该任务,task identifier==%@",identifier);
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"任务池中不存在该任务", LXTaskCRUDResultTypeNotExist);
        }
    }
}
...
- (void)p_actionForSuspendTaskIdentifier:(NSString*)identifier {
    ...
    if (success) {
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"提醒⚠️:进行中任务池暂停此任务成功", LXTaskCRUDResultTypeSuspendSuccess);
        }
        return;
    }
    ...
    if (success) {
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"提醒⚠️:等待中任务池暂停此任务成功", LXTaskCRUDResultTypeSuspendSuccess);
        }
        return;
    }
    ...
    if (success) {
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"提醒⚠️:暂停中任务池已存在此任务", LXTaskCRUDResultTypeExist);
        }
        return;
    }
    
    if (self.taskPoolActionCallback) {
        self.taskPoolActionCallback(@"提醒⚠️:任务池不存在此任务", LXTaskCRUDResultTypeNotExist);
    }
}
- (void)p_actionForResumeTaskIdentifier:(NSString*)identifier {
    ...
    if (success) {
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"提醒⚠️:进行中任务池已存在此任务", LXTaskCRUDResultTypeExist);
        }
        return;
    }
    ...
    if (success) {
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(@"提醒⚠️:等待中任务池已存在此任务", LXTaskCRUDResultTypeResumeSuccess);
        }
        return;
    }
    ...
    if (success) {
        if (self.taskPoolActionCallback) {
            self.taskPoolActionCallback(msg, resultType);
        }
        return;
    }
...
- (void)p_actionForCompletedTask:(LXHttpTaskModel*)task responseData:(LXHttpResData*)responseData {
    ...
    if (self.taskCompletedCallback) {
        self.taskCompletedCallback(_task, responseData);
    }
}
- (void)p_actionForUploadProgressTask:(LXHttpTaskModel*)task progress:(NSProgress*)uploadProgress {
    if (self.taskUploadProgressCallback) {
        self.taskUploadProgressCallback(task, uploadProgress);
    }
}
- (void)p_actionForDownloadProgressTask:(LXHttpTaskModel*)task progress:(NSProgress*)downloadProgress {
    if (self.taskDownloadProgressCallback) {
        self.taskDownloadProgressCallback(task, downloadProgress);
    }
}
...
- (LXHttpTaskPool * _Nonnull (^)(void (^ _Nullable)(NSString * _Nonnull, LXTaskCRUDResultType)))lx_taskPoolActionCallback {
    return ^(void (^ _Nullable taskPoolActionCallback)(NSString * _Nonnull, LXTaskCRUDResultType)) {
        self.taskPoolActionCallback = taskPoolActionCallback;
        return self;
    };
}
...
- (LXHttpTaskPool* (^)(void (^taskUploadProgressCallback)(LXHttpTaskModel* _Nonnull task, NSProgress* _Nonnull progress)))lx_taskUploadProgressCallback {
    return ^(void (^taskUploadProgressCallback)(LXHttpTaskModel* _Nonnull task, NSProgress* _Nonnull progress)) {
        self.taskUploadProgressCallback = taskUploadProgressCallback;
        return self;
    };
}
- (LXHttpTaskPool* (^)(void (^taskDownloadProgressCallback)(LXHttpTaskModel* _Nonnull task, NSProgress* _Nonnull progress)))lx_taskDownloadProgressCallback {
    return ^(void (^taskDownloadProgressCallback)(LXHttpTaskModel* _Nonnull task, NSProgress* _Nonnull progress)) {
        self.taskDownloadProgressCallback = taskDownloadProgressCallback;
        return self;
    };
}
